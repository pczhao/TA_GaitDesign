...
% Load model
rabbit = RABBIT('../Utils/urdf/five_link_walker.urdf');
...
rabbit_1step = addVertex(rabbit_1step, 'Stance_Vtx1', 'Domain', r_stance);
...
if ~isempty(load_path)
    nlp.configure(mybounds, 'LoadPath',load_path);
else
    nlp.configure(mybounds, p1, p2);
end

 
% Add costs
addRunningCost(nlp.Phase(getPhaseIndex(nlp,'Stance_Vtx1')),u2r_fun,'u');
