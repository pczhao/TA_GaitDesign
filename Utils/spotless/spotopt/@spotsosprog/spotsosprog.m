...
classdef spotsosprog < spotprog
    ...
    methods (Access = protected)
        ...
        function [pr,Q,phi,y,basis] = buildSDSOSDecompPrimal(pr,expr,options)
            if ~spot_hasSize(expr,[1 1])
                error('buildSOSDecomp expects a scalar polynomial.');
            end

            phi = spotsosprog.buildGramBasis(expr,pr.variables,options);

            [pr,Q] = pr.newSDD(length(phi));
            sosCnst = expr-phi'*Q*phi;
...
% keyboard;
	...
	function [pr,Q,phi,y,basis] = buildDiagSOSDecompPrimal(pr,expr)
            if ~spot_hasSize(expr,[1 1])
                error('buildSOSDecomp expects a scalar polynomial.');
            end

            phi = spotsosprog.buildGramBasis(expr,pr.variables);

            [pr,Q] = pr.newDiag(length(phi));
            sosCnst = expr-phi'*Q*phi;
