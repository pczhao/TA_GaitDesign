...
%         leg2Color = 'b';
        leg2Color = [0    0.4470    0.7410] * 0.3 + 0.7;
        torsoColor = [ 0.7 0.7 0.7 ];
        groundColor = 'g';
    end
...
%             obj.ground = surf(terrain.Tx,terrain.Ty,terrain.Tz-0.025,'LineWidth',6, 'EdgeColor', [0.7,0.7,0.7]); hold on;
            if nargin > 2
                obj.target = plot3(0*target, target, 0*target, 'x', 'MarkerSize', 10, 'LineWidth', 4);
            end
...
%                 obj.text = text(0,pH(1),1.5,['t = ',sprintf('%.2f',t)]);
                obj.text = text(0,pH(1),1.5,[sprintf('%.2f',t)]);
...
%             center = [0; q(1:2)];
            center = [ 0; q(1); 0.7 ];
%             radius = 1.5;
%             radius = [-1,1,    -0.5,    6,     -0.2, 1.5];
            radius = [ -1,1, -0.5, 11, -0.2, 1.8 ];
            yaw = 0;
        end
    end
end
